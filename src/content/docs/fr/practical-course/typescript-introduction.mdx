...
Le type est un ensemble d'éléments uiq permettent de catégoriser l'information.
Dans cet exemple, non seulement nous lions les différentes caractéristiques d'une voiture comme son modèle ou son constructeur, mais nous définissons également le type même de chaque propriété.
Nous manipulerons donc des strings pour le modèle et le constructeur mais un nombre pour l'année.
