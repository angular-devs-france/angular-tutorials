...
It provides a **Router** API to modify content based on the URL.

The project currently displays a list of tasks.
In order to add a new task to this list, you need to be able to switch from the list view to the form view.

## Router-outlet directive

The **RouterModule** provides a directive called `router-outlet`. This directive is used to display the content of the current route. When the user navigates to a different route, the content of the `router-outlet` is replaced with the content of the new route.
...
In the above case, the `router-outlet` is placed between the `<main>` tags. This means that you will continue to have the `<header>` on all pages and the configured content will change based on the URL you navigate to.
